# Live Demonstration UX Report: `codex-foreman`

Date: 2026-03-01  
Scope: Live audit demo of `codex-foreman` with parallel workers, run via `/projects`, `/projects/{id}/workers`, and `GET /agents/{id}` polling.
...
- Created an audit project under `/home/cmd/repos/codex-foreman/.project`
- Used:
...
`codex-foreman` is functionally capable but not yet agent-ergonomic for:
...
2. Add `wait` primitives
   - `GET /workers/{id}/wait` with optional timeout and poll hints, or CLI `codex-foreman wait <id>`.
...
5. Add run mode for CLI
   - `codex-foreman run ... --model ... --template ... --wait --emit-json`.
...
7. Surface service/process status in API
   - `codex-foreman status` CLI/endpoint with PID, uptime, last event, restart reason.
