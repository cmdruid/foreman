...
pub async fn wait_for_http(base_url: &str) {
...
pub async fn start_foreman(
    ...
    let foreman_binary = binary_path("codex_foreman");
    ...
    wait_for_http(&base_url).await;

    ForemanHarness { base_url, child }
}
...
pub fn binary_path(name: &str) -> String {
    let normalized_names = vec![name.to_string(), name.replace('_', "-")];
...
fn fallback_binary(name: &str) -> String {
    let normalized_names = [name.to_string(), name.replace('_', "-")];
