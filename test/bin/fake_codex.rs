...
fn main() {
    ...
    // The test binary requires the invocation to be `app-server` and ignores flags after it.
...
fn run_stub_server() {
    ...
    let mut thread_stall_state = HashSet::new();
        ...
        match method {
            ...
            "turn/start" => {
                ...
                send_notification(
                    &mut stdout,
                    "turn/started",
                    json!({"threadId": thread_id, "turnId": turn_id, "status": "running"}),
                );
                ...
                if !should_stall {
                    send_notification(
                        ...
                        json!({"threadId": thread_id, "turnId": turn_id, "command": "mock-cmd"}),
                    );
                    send_notification(
                        &mut stdout,
                    ...
                    send_notification(
                        &mut stdout,
                        "turn/completed",
                        json!({"threadId": thread_id, "turnId": turn_id, "status": "completed", "items": [{"type": "text", "text": "worker complete"}]}),
                    );
                }
            }
            "turn/steer" => {
                let thread_id = parse_string_param(&params, "thread")
                    .unwrap_or_else(|| "thread-unknown".to_string());
                let expected_turn_id = parse_string_param(&params, "expectedTurnId")
                ...
                send_response(
                    &mut stdout,
                    &id,
                    json!({"turnId": turn_id, "expectedTurnId": expected_turn_id}),
                );
                ...
                send_notification(
                    &mut stdout,
                    "turn/completed",
                    json!({"threadId": thread_id, "turnId": turn_id, "status": "completed", "items": [{"type": "text", "text": "steered"}]}),
                );
            }
            "turn/interrupt" => {
                let thread_id = parse_string_param(&params, "thread")
...
fn parse_string_param(params: &Value, stem: &str) -> Option<String> {
    ...
    if let Some(value) = params.get(key).and_then(parse_id_value) {
        return Some(value);
    }
