...
description: Teaches agents how to use foreman for worker orchestration, callback handling, and project workflows.
metadata:
  short-description: Operate foreman foreman/worker model and callbacks.
---

# Codex-Foreman Orchestrator

Use this skill when you need to start, monitor, steer, and complete workflows through `foreman`.

## Core model

- `foreman` runs a long-lived foreman service.
- A **foreman** agent orchestrates one project and can spawn **worker** agents.
- Workers and the foreman are controlled via unix socket endpoints and callback profiles.
...
Install this skill path into your Codex skill directory (`$CODEX_HOME/skills/foreman`).

```bash
mkdir -p "$CODEX_HOME/skills"
cp -r contrib/skill "$CODEX_HOME/skills/foreman"
```

For OpenClaw-compatible runtimes, install as the equivalent local skill bundle folder; keep the folder name `foreman` to make intent explicit.

## Runtime baseline

Before issuing agent-facing instructions, verify the service target:

- Foreman endpoint reachable via unix socket (example socket: `FOREMAN_SOCKET_PATH=./.foreman/foreman.sock`)
...
```bash
foreman --init-project /path/to/project --init-project-manual
```
...
```bash
foreman --service-config /etc/foreman/config.toml --project /path/to/project/project.toml
```

```bash
export FOREMAN_SOCKET_PATH="./.foreman/foreman.sock"
