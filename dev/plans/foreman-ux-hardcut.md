...
Date: 2026-03-01  
Repository: `foreman`  
Goal: Make `foreman` easier to use for autonomous agents by replacing ad-hoc event parsing with stable result/wait semantics and batch orchestration primitives.

## Principles

- Hard-cut changes only; no alternate execution shims.
...
- [ ] Parse and capture:
  ...
  - file refs (best-effort extraction, fallback to captured text)
...
- [ ] Keep existing free-form output.
...
- [ ] Update/fix fixtures if needed in `test/fixtures`.
...
- [ ] Extend CLI (likely `src/main.rs` CLI parser/subcommands):
  - `foreman run ... --wait --emit-json`
  - `foreman run-project --path ... --template ... --model ... --wait`
  - `foreman wait <agent-id> [--timeout] [--json]`
...
- [ ] CLI option:
  - `foreman run ... --batch-label "security" --batch-label "robustness" ...`
...
- [ ] Use diagnostics without full event log dumps.
...
- [ ] Add:
  - `foreman status`
  - `foreman stop`
  - `foreman restart` (hard-cut: managed process wrapper where feasible)
...
- [ ] Refactor callback handling in `src/foreman.rs`:
  - one unified path for webhook/command/profile/url with shared timeout/error behavior.
